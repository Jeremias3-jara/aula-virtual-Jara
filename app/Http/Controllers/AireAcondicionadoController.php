...
class AireAcondicionadoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            // Usamos 'aires_acondicionados' como nombre de tabla para la unicidad
     ...
     */
    public function show(AireAcondicionado $airesacondicionado) // Usamos 'airesacondicionado' como variable
    {
        // Carga la relación 'aula' antes de retornar el modelo
        return $airesacondicionado->load('aula');
    }
     ...
     */
    public function update(Request $request, AireAcondicionado $airesacondicionado) // Usamos 'airesacondicionado' como variable
    {
        // 1. Validar los datos de entrada para la actualización
        $validated = $request->validate([
            'marca' => 'sometimes|required|string|max:255',
            // La validación 'unique' debe ignorar el ID del registro actual
            'modelo' => 'sometimes|required|string|max:255|unique:aires_acondicionados,modelo,' . $airesacondicionado->id,
            'aula_id' => 'sometimes|required|integer|exists:aulas,id',
        ]);

        // 2. Actualizar el registro
        $airesacondicionado->update($validated);

        // 3. Retornar el recurso actualizado
        return response()->json($airesacondicionado->load('aula'), 200);
    }
     ...
     */
    public function destroy(AireAcondicionado $airesacondicionado) // Usamos 'airesacondicionado' como variable
    {
        $airesacondicionado->delete();
